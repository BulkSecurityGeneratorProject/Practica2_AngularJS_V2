...
public class JugadorResource {
    ...
    //jugadores asistencias Ex2
    @RequestMapping(value = "/topAsistents/{asistencias}",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<List<Jugador>> topAsistents(@PathVariable Integer asistencias, Pageable pageable)
        throws URISyntaxException {
        Page<Jugador> page = jugadorRepository.topAsistents(asistencias, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/jugadors/topAsistents");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
}
